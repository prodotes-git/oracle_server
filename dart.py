...
async def fetch_order_backlog_data(corp_code: str, year: str, reprt_code: str):
    """fnlttSinglAcntAll API를 사용하여 수주잔고 관련 계정 조회
    계약자산, 계약부채, 미청구공사 등을 추출합니다."""
    ...
    res_data = {"contract_asset": 0, "contract_liability": 0, "order_backlog": 0}
    
    # 계약자산/부채 관련 키워드
    asset_keywords = ["계약자산", "미청구공사", "미청구채권", "미청구수익", "공사미수금"]
    liability_keywords = ["계약부채", "초과청구공사", "선수금", "선수수익"]
    backlog_keywords = ["수주잔고", "수주잔액", "미이행수주잔고"]
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if data.get("status") != "000":
                    ...
                    res = await client.get(url_ofs, timeout=15.0)
                    if res.status_code == 200:
                        data = res.json()
                ...
                if data.get("status") == "000":
                    ...
                    for item in lst:
                        ...
                        # 재무상태표(BS) 또는 주석(SCE) 항목만
                        if sj_div not in ["BS", "SCE", "IS", "CIS", ""]:
                            continue
                        ...
                        # 계약자산 계열
                        if any(kw in acc_nm for kw in asset_keywords):
                            # 더 큰 값을 사용 (세부 항목 vs 총액)
                            if abs(amount) > abs(contract_asset):
                                contract_asset = amount
                        
                        # 계약부채 계열
                        elif any(kw in acc_nm for kw in liability_keywords):
                            if abs(amount) > abs(contract_liability):
                                contract_liability = amount
                        
                        # 수주잔고 직접 키워드
                        elif any(kw in acc_nm for kw in backlog_keywords):
                            if abs(amount) > abs(order_backlog):
                                order_backlog = amount
                    
                    res_data["contract_asset"] = contract_asset
                    res_data["contract_liability"] = contract_liability
                    res_data["order_backlog"] = order_backlog
        except Exception as e:
            print(f"fetch_order_backlog error: {e}")
...
async def get_stats(corp_code: str):
    ...
    for year, code, label in reprts:
        cache_key = f"dart_stats_v6:{corp_code}:{year}:{code}"
